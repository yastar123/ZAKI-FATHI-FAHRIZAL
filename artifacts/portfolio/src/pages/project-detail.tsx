...
const BADGE_ACCENT: Record<
  string,
  { badge: string; color: string; bg: string; border: string; bar: string }
> = {
  "CFD Analysis": {
    badge: "CFD Analysis",
    color: "text-blue-700",
      ...
      {images.length > 1 && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white z-10"
          >
            <ChevronLeft size={20} />
          </button>
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white z-10"
          >
        ...
        >
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-end p-2">
            <div className="opacity-0 group-hover:opacity-100 transition-opacity w-full">
              <p className="text-white text-[10px] leading-snug line-clamp-2 text-left">
                {img.caption}
              </p>
            </div>
          </div>
...
export default function ProjectDetailPage() {
  ...
  const normalizedProjectId = String(projectId ?? "").replace(/^0+/, "") || "";
  const projectIndex = allProjects.findIndex(
    (p) => String(p.id).replace(/^0+/, "") === normalizedProjectId,
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-24 pb-20">
        ...
        >
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span
              className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${accent.bg} ${accent.color} ${accent.border}`}
            >
              {accent.badge}
            </span>
          ...
          {project.tools && project.tools.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {project.tools.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-1.5 tag-gray text-xs"
                >
                  <Tag size={11} />
                  {t}
                </span>
        ...
        >
                ...
                <ol className="space-y-3">
                  {project.method_steps.map((step, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span
                        className={`flex-shrink-0 w-6 h-6 ${accent.bar} text-white text-xs font-bold rounded-full flex items-center justify-center mt-0.5`}
                      >
                        {i + 1}
                      </span>
                      <p className="text-sm text-foreground/80 leading-relaxed">
                ...
                <Section label="Results">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {project.results.map((r) => (
                      ...
                      >
                        <div
                          className={`text-xs font-mono ${accent.color} mb-2`}
                        >
                          {r.number}
                        </div>
                        ...
                        <div
                          className={`text-xs font-semibold ${accent.color} ${accent.bg} px-2.5 py-1 rounded-full inline-block`}
                        >
                          {r.metric}
                        </div>
                      </div>
          ...
          <div className="space-y-5">
            {project.result && (
              <div
                className={`${accent.bg} border ${accent.border} rounded-xl p-5`}
              >
                <p
                  className={`text-xs font-semibold uppercase tracking-wider ${accent.color} mb-2`}
                >
                  Key Outcome
                </p>
                <p className="text-sm text-foreground/80 leading-relaxed">
            ...
            {project.tools && project.tools.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tools.map((t) => (
                    <span key={t} className="tag-gray text-xs">
                      {t}
                    </span>
                  ))}
                </div>
